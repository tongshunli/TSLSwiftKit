...
class TSLLocalizedManager: NSObject {
    ...
    lazy var languageBundle: Bundle? = {
        let path = Bundle.main.path(forResource: TSLLocalizedManager().userLanguageName, ofType: "lproj")
        var languageBundle = Bundle(path: path ?? "")
        return languageBundle
    }()
    
    //  用户本地语言,如果语言包中没有,返回默认值
    lazy var userLanguageName: String = {
        var userLanguageName = UserDefaults.standard.string(forKey: TSL_User_Language)
        
        return userLanguageName ?? TSLLocalizedManager.StringFromLocalizedLanguage(.defaultLanguage)
    }()
    
    //  设置默认语言
    public func setUpDefaultLanguage(_ language: String) -> Void {
