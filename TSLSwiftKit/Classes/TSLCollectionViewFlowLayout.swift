...
public class TSLCollectionViewFlowLayout: UICollectionViewFlowLayout {
    ...
    func layoutAttributesForElementsInRect(_ rect: CGRect) -> NSArray {
        ...
        for index in 0..<layoutAttributes.count {
            // MARK: 当前cell的位置信息
            guard let currentAttr = layoutAttributes[index] as? UICollectionViewLayoutAttributes else { return layoutAttributes }
            // MARK: 上一个cell的位置信
            ...
            self.sumCellWidth += Double(currentAttr.frame.size.width)
    ...
    func cellFrameWith(_ layoutAttributes: NSMutableArray) {
        ...
        case .center:
            let totalCount = layoutAttributes.count - 1
            let totalBet = Double(totalCount) * (self.betweenOfCell ?? 0.0)
